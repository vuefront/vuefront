...
export const getCategory = () => ({
  ...
  categories: [getSubCategory(), getSubCategory(), getSubCategory(), getSubCategory()]
});
...
export const getManufacturers = count => {
  let result = [];
...
export const getMenuLink = () => {
  ...
  for (let i = 0; i < faker.datatype.number({
    ...
    for (let j = 0; j < faker.datatype.number({
      min: 0,
      max: 5,
      precision: 1
    }); j++) {
...
//# sourceMappingURL=fakeData.mjs.map