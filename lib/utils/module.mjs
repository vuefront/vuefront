...
export default function useModule() {
  const vuefront$ = inject("$vuefront");
    ...
    if (!status) {
      for (const route in vuefront$.layouts) {
        ...
        if (regex.test(currentRoute.value)) {
          ...
          if (!isUndefined(layout.extensions) && !isUndefined(layout.extensions[position]) && !isEmpty(layout.extensions[position])) {
            result = true;
            break;
          }
        }
