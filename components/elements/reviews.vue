    ...
    <div class="mb-4">
      <v-card v-for="(review, index) in reviews" :key="index" class="mb-3">
        <v-card-title primary-title>
          <div>
            <div class="reviews-section__author subheading font-weight-medium mb-1">
              {{review.author}}
            </div>
            <div class="review-section_content body-1">
              {{review.content}}
            </div>
          </div>
        </v-card-title>
...
export default class extends Vue {
  ...
  handleSend() {
    this.$emit('create', {content: this.review, author: this.author, rating: this.rating})
    this.author = ''
    this.rating = 0
    this.review = ''
  }
